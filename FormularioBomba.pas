...
begin
  ...
  begin
    if (Trim(cboTanque.Text) = '') or (Trim(edtNome.Text) = '') or (edtValor.Text = ' ,   ') then
    begin
  ...
  begin
    if (Trim(edtNome.Text) = '') or (edtValor.Text = ' ,   ') then
    begin
      Application.MessageBox(PChar('Todos os campos são obrigatório.'),'Atenção',MB_OK + MB_ICONINFORMATION);
    end
    else
...
var bombainc : TBomba;
begin
  if bombainc.Inclui (cboTanque.KeyValue, edtNome.Text, edtValor.Text) then
  begin
    Application.MessageBox(PChar('Inclusão realizada com sucesso.'),'Atenção',MB_OK + MB_ICONINFORMATION);
    grdPrincipal.DataSource := bombainc.Lista;
    edtNome.Clear;
    edtValor.Clear;
...
var bombaalt : TBomba;
begin
  if bombaalt.Altera (grdPrincipal.DataSource.DataSet.FieldByName('Codigo').AsString, edtNome.Text, edtValor.Text) then
  begin
    Application.MessageBox(PChar('Alteração realizada com sucesso.'),'Atenção',MB_OK + MB_ICONINFORMATION);
    grdPrincipal.DataSource := bombaalt.Lista;
    edtValor.Clear;
    edtNome.Clear;
    HabilitaCampos(False);
  end
...
var bomba : TBomba;
begin
  if bomba.PodeExcluir(grdPrincipal.DataSource.DataSet.FieldByName('Codigo').AsString) then
  begin
    if (Application.MessageBox(PChar('Confirma a exclusão da bomba?'),'Confirmação',MB_YESNO + MB_ICONQUESTION) = IDYES) then
    begin
      if bomba.Exclui(grdPrincipal.DataSource.DataSet.FieldByName('Codigo').AsInteger) then
      begin
